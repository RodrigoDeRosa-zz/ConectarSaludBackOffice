...
export class TableFilteredComponent implements OnInit {
   ...
   */
  @Input('filterFields') filterFields: ABMGenericFormField[];
  /*
   * entrada que cuando los filtros estan activos, renderea botones al lado del submit
   */
  @Input('extraFilterButtons') extraFilterButtons: ABMGenericFormField[];
  /*
  ...
  ngOnInit() {
    ...
    if(this.showFilters){
      ...
      this.filterFields.push(new ABMGenericFormField({name: 'submit', value: '', type:'submit', title:'Buscar', size:'span-1'}));
